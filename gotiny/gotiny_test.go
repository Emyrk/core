...
func init() {
	...
	for i := 0; i < length; i++ {
		...
		// Use reflect to get the underlying pointer safely (compatible with checkptr)
		srcp[i] = tempi.UnsafePointer()
		retp[i] = tempv.UnsafePointer()
	}
...
func TestGetName(t *testing.T) {
	...
	items := []struct {
		...
		{"int", int(1)},
		{"github.com/cybriq/gotiny.Encoder", gotiny.Encoder{}},
		{"*int", (*int)(nil)},
		{"**int", (**int)(nil)},
		{"[]int", []int{}},
		{"[]time.Time", []time.Time{}},
		{"[]github.com/cybriq/gotiny.GoTinySerializer", []gotiny.GoTinySerializer{}},
		{"*interface {}", (*interface{})(nil)},
		{"map[int]string", map[int]string{}},
		{"struct { a struct { int; b int; dec []github.com/cybriq/gotiny.Decoder; abb interface {}; c io.ReadWriteCloser } }",
			struct {
				a struct {
					int
					b   int
